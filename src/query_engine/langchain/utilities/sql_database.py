...
class SQLDatabase:
    ...
    def __init__(
        ...
        self._all_tables = set(
            self._inspector.get_table_names(schema=(None if schema == '' else schema))
            + (self._inspector.get_view_names(schema=(None if schema == '' else schema)) if view_support else [])
        )
        ...
        self._metadata = metadata or MetaData(bind=self._engine, schema=(None if self._schema == '' else self._schema))
        # including view support if view_support = true
        self._metadata.reflect(
            views=view_support,
            bind=self._engine,
            only=list(self._usable_tables),
            schema=(None if self._schema == '' else self._schema),
        )
